...
def example_usage():
    ...
    # Create the joint analyzer
    knee_analyzer = KneeJointAnalyzer(femur_landmarks, tibia_landmarks)

    # Example of update with new positions (in a real application, these would come from motion capture)
    # Simulating a small movement
    femur_current_markers = {
        'proximal': [77.5, -127.6, 911.7],  # Slightly moved
        'distal': [65.5, -113.2, 1385.0],   # Slightly moved
        'lateral': [67.2, -157.8, 1399.6],  # Slightly moved
        'medial': [83.4, -106.3, 1398.1]    # Slightly moved
    }

    tibia_current_markers = {
        'proximal': [89.9, -127.6, 1402.1], # Slightly moved
        'distal': [53.4, -97.0, 1782.2],    # Slightly moved
        'lateral': [58.2, -146.5, 1406.6],  # Slightly moved
        'medial': [100.5, -102.9, 1403.6]   # Slightly moved
    }

    # Calculate joint angles
    angles = knee_analyzer.update_transformations(
        femur_current_markers, tibia_current_markers)
    
    # Display angles
    print(f"Flexion: {angles['flexion']:.2f}°")
    print(f"Varus/Valgus: {angles['varus_valgus']:.2f}°")
    print(f"Rotation: {angles['rotation']:.2f}°")
